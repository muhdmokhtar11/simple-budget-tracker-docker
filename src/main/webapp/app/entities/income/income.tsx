...
export const Income = () => {
  ...
  return (
    <div>
      <h2 id="income-heading" data-cy="IncomeHeading">
        Incomes
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
          </Button>
          <Link to="/income/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
      ...
      <div className="table-responsive">
        {incomeList && incomeList.length > 0 ? (
            ...
            <tbody>
              {incomeList.map((income, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/income/${income.id}`} color="link" size="sm">
                      {income.id}
                    </Button>
                  </td>
                  <td>{income.amount}</td>
                  <td>{income.description}</td>
                  <td>{income.date ? <TextFormat type="date" value={income.date} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{income.category ? <Link to={`/category/${income.category.id}`}>{income.category.name}</Link> : ''}</td>
                  <td>{income.user ? income.user.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/income/${income.id}`} color="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && <div className="alert alert-warning">No Incomes found</div>
        )}
      </div>
      {totalItems ? (
        <div className={incomeList && incomeList.length > 0 ? '' : 'd-none'}>
